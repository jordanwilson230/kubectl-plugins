...
if [[ "$KUBECTL_PLUGINS_LOCAL_FLAG_FILE" =~ "external-dns" ]]; then
   ...
   cat "$KUBECTL_PLUGINS_LOCAL_FLAG_FILE" | sed -e "s|Pixie|$cluster|g; s|current-context: \(.*\)|current-context: $context|g" | kubectl -n default apply -f - $KUBECTL_PLUGINS_LOCAL_FLAG_DRY
   exit 0
fi
